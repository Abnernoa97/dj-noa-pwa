    ...
    <section className="page-card calendar-page">
      ...
      <div className="calendar-grid">
        {days.map((day) => {
          ...
          return <div key={dateKey} className={`calendar-day ${outside ? 'outside' : ''} ${isSameDay(day, new Date()) ? 'today' : ''}`}><span>{format(day, 'd')}</span>{matches.slice(0, 2).map((event) => <button className="calendar-event-button" key={event.id} onClick={() => onOpenEvent(event)}>{event.title}</button>)}</div>;
        })}
      </div>
      <p className="section-note">Toca un evento para editarlo. O di: “mueve el próximo evento al 28/09/2026”.</p>
    </section>
  );
}
